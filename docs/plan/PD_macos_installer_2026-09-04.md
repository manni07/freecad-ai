...
Final automated evidence after PR-CI correction is `69 passed in 18.30s` for the installer suite, `375 passed in 46.43s` for the exact CI-equivalent security slice, and `1625 passed in 150.79s` for all unit tests, all with zero skips. The final independent scoped security review passed at 98.4%. These are local gates; GitHub-hosted CI remains `HOLD` until the correction commit is pushed and the latest-head checks pass. Live FreeCAD loading also remains `HOLD`; it was not started or represented as tested.
