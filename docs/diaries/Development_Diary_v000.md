...
- The first GitHub security-regression run failed during collection because its pinned test-tool installation omitted PySide while two selected security tests import the UI compatibility layer. The CI-only test dependency was pinned to the locally verified `PySide6==6.11.2`; runtime dependencies remain empty because FreeCAD supplies the GUI binding in production.
