...
- Additional bounded weekly compatibility: the same addon GUI passed with13 source hashes and exit0;10 native preflight cases passed13.45s via private candidate binary/profile routing without source edits. Initial private driver collection failure was corrected by a module wrapper; full candidate suite is NOT RUN. Public commit a5752dc and PR#4 track publication; final merge state is recorded by the PR.

## 2026-09-06 — Approved weekly activation and live MCP acceptance

- User explicitly approved the proposed targeted restart into tested weekly26.3.0 after backups. The prior restart quote came from the user-pasted “AGENTS.md instructions”, not a filesystem file; checked ancestor/relevant subtree paths contained none. No second generic approval was requested for the same approved target.
- Five distinct backups passed independent candidate reopen, CRC/XML/hash/object/link/GUI/state checks and five camera-framing comparisons before shutdown. Actual structured save temporarily repointed FileName and marked all five modified; this was not an identity-preserving save operation. Bootstrap restored recorded Name/Label/original FileName/Modified/active/count exactly, including the empty document and original two-modified/three-unmodified split. Original files were not overwritten; original/backup hashes unchanged. Undo history is not restored by restart.
- Root stopped only the identified old FreeCAD process after backup validation. First new startup failed before restoration/MCP because built-in App lacks __file__; root closed that failed owned process and switched the guard to the native home-path API. Next launch passed and remains running as FreeCAD26.3.0/build20260902/c68c452.
- LIVE_ACTIVATION_PASS:15 source files/13 loaded modules matched;55 authenticated tools with complete schema parity and unauthenticated401; token unchanged; five document states restored; toolbar checked/chat visible; INFO/default10pt font correct; three ordinary MCP startup lines present without original Sans/read/corruption warnings. AI Python remains OFF and optimized preflight ON.
- Viewport difference23,690 pixels was confined to the navigation cube; zero changed pixels outside the overlay. Supervisor visually confirmed the same wall/eight rows. Independent backup validation passed all five camera framings; images are not byte-identical.
- Parent changes are limited to the current HTML manual/STP, concise activation report and this diary append. Exact private evidence remains under .integration-evidence/activation-20260906/. Generic launcher still defaults to stable; this explicit weekly profile/bootstrap does not change default-app associations. Extra recovery path variants, exhaustive geometry and full candidate suite remain NOT RUN. New documentation publication is root-owned.
